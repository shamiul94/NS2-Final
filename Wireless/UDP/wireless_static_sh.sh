...
coverArea=$coverAreaInit

iteration=$(printf %.0f $iteration_float);

#echo 'PLease enter # of nodes, # of flows, packet rate, and coverArea'
#read nNodes nFlows pcktRate coverArea

echo 'Simulation for 802.11 (Static)'
...
do
	...
	do
		...
		ns $tclFile $nNodes $nFlows $pcktRate $coverArea
		echo "SIMULATION COMPLETE. BUILDING STAT......"

		awk -f wireless_static_awk.awk my_wireless_static_trace.tr > $tempFile
	...
	echo "coverArea:                        $coverArea " >> $output_file
...
######################
	...
	if [ "$param" == "1" ]; then
		...
		echo -ne "$coverArea " >> $graphData
		coverArea=$(($coverAreaInit*$round))
	fi
