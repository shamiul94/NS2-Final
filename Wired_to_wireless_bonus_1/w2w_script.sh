...
outputDirectory="output_wired_cum_wireless/"
...
do
	...
	do
		...
		awk -f final_w2w_awk.awk wired-and-wireless.tr > $tempFile
		cp wired-and-wireless.tr $flnm
...
while [ $i -ge 2 ]
do
	gnuplot -persist -e "set terminal png size 700,500; set output '$outputDirectory${arr[$i]}VS$param.png';set title 'Wired-cum-Wireless : ${arr[$i]} vs $param'; set xlabel '$param'; set ylabel '${arr2[$i]}'; plot '$outputDirectory/GRAPH' using 1:$i with lines"
	i=$(($i-1))
done
